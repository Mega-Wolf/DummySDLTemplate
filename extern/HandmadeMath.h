...
#if defined(HANDMADE_MATH__USE_SSE) && !defined(NO_HEADERS)
#include <xmmintrin.h>
#endif
...
COVERAGE(HMM_Lerp, 1)
HMM_INLINE float HMM_PREFIX(Lerp)(float A, float B, float Time)
{
...
HMM_INLINE hmm_mat4 HMM_PREFIX(Translate)(float x, float y, float z)
{
    return HMM_PREFIX(Translate)({x, y, z});
}

HMM_EXTERN hmm_mat4 HMM_PREFIX(Rotate)(float Angle, hmm_vec3 Axis);
HMM_EXTERN hmm_mat4 HMM_PREFIX(RotateRadians)(float AngleRadians, hmm_vec3 NormalisedAxis);
...
HMM_INLINE hmm_mat4 HMM_PREFIX(Scale)(float x, float y, float z)
{
    return HMM_PREFIX(Scale)({x, y, z});
}
...
#else
    Result.X = HMM_PREFIX(Lerp)(Left.X, Right.X, Time);
    Result.Y = HMM_PREFIX(Lerp)(Left.Y, Right.Y, Time);
    Result.Z = HMM_PREFIX(Lerp)(Left.Z, Right.Z, Time);
    Result.W = HMM_PREFIX(Lerp)(Left.W, Right.W, Time);
#endif
...
COVERAGE(HMM_RotateRadians, 1)
hmm_mat4 HMM_PREFIX(RotateRadians)(float Angle, hmm_vec3 NormalisedAxis)
{
    ASSERT_COVERED(HMM_RotateRadians);
    ...
    Result.Elements[0][0] = (NormalisedAxis.X * NormalisedAxis.X * CosValue) + CosTheta;
    Result.Elements[0][1] = (NormalisedAxis.X * NormalisedAxis.Y * CosValue) + (NormalisedAxis.Z * SinTheta);
    Result.Elements[0][2] = (NormalisedAxis.X * NormalisedAxis.Z * CosValue) - (NormalisedAxis.Y * SinTheta);

    Result.Elements[1][0] = (NormalisedAxis.Y * NormalisedAxis.X * CosValue) - (NormalisedAxis.Z * SinTheta);
    Result.Elements[1][1] = (NormalisedAxis.Y * NormalisedAxis.Y * CosValue) + CosTheta;
    Result.Elements[1][2] = (NormalisedAxis.Y * NormalisedAxis.Z * CosValue) + (NormalisedAxis.X * SinTheta);

    Result.Elements[2][0] = (NormalisedAxis.Z * NormalisedAxis.X * CosValue) + (NormalisedAxis.Y * SinTheta);
    Result.Elements[2][1] = (NormalisedAxis.Z * NormalisedAxis.Y * CosValue) - (NormalisedAxis.X * SinTheta);
    Result.Elements[2][2] = (NormalisedAxis.Z * NormalisedAxis.Z * CosValue) + CosTheta;
